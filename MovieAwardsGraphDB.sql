...
--Найти все выигранные награды для Шона Бэйкера и связанных с ним сущностей через участие в фильмах.
SELECT 
    Person.name AS [Участник],
    STRING_AGG(Movie.title, ' -> ')  WITHIN GROUP (GRAPH PATH) AS [Фильмы],
    STRING_AGG(Award.category, ', ')  WITHIN GROUP (GRAPH PATH) AS [Награды]
FROM 
    People AS Person,
    ParticipatedIn FOR PATH AS pi,
    Movies FOR PATH AS Movie,
    Won FOR PATH AS w,
    Awards FOR PATH AS Award
WHERE 
    MATCH(SHORTEST_PATH(
        Person(-(pi)->Movie 
              -(w)->Award)+
    ...
    AND Person.name = N'Шон Бэйкер'


-- Найти связи между Шоном Бэйкером и другими людьми через участие в фильмах (максимум 3 шага).
;
WITH PathData AS (
    SELECT
        Person1.name  AS FromPerson,
        LAST_VALUE(Person2.name) WITHIN GROUP (GRAPH PATH) AS ToPerson,
        STRING_AGG(Movie.title, N' -> ') WITHIN GROUP (GRAPH PATH) AS Path
    FROM
        People AS Person1,
        ParticipatedIn FOR PATH AS pi1,
        Movies FOR PATH AS Movie,
        ParticipatedIn FOR PATH AS pi2,
        People FOR PATH AS Person2
    WHERE 
        MATCH(SHORTEST_PATH(Person1(-(pi1)->Movie<-(pi2)-Person2){1,3}))
        AND Person1.name = N'Шон Бэйкер'
)
SELECT 
    FromPerson AS [От кого],
    ToPerson AS [К кому],
    COUNT(*) AS [Количество связей],
    Path AS [Через фильмы]
FROM PathData
GROUP BY FromPerson, ToPerson, Path;
